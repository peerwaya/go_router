...
1. The `@TypedGoRoute` annotation:
   - It's only valid to annotate implementations of `GoRouteData`.
     - May want to expand this pretty soon, to include Widgets? Libraries?
       Should discuss.
...
2. `GoRouteData` implementations
   ...
   - All constructor parameters must map 1-to-1 name and type to fields or
     getters on the type.
     - Question: do we want to support read/write fields and getter/seter pairs?
   - Only fields that align getters with constructor parameters or setters will
   ...
   - Valid parameter/field/property types:
     ...
     - `num`, `int`, `bool`, `DateTime`, `Duration`, `BigInt`, `double`, `Uri`:
       encoded with `toString`
     - `Enum`: encoded using kebab cased names.
